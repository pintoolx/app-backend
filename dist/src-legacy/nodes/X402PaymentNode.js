...
import { createTransferInstruction, getOrCreateAssociatedTokenAccount, createAssociatedTokenAccountInstruction, getAccount, } from "@solana/spl-token";
 ...
 */
export class X402PaymentNode {
    description = {
        displayName: "X402 Payment",
        name: "x402Payment",
        group: ["payment"],
        version: 1,
        description: "Make payment-required requests using x402 protocol (Solana USDC payments)",
        inputs: ["main"],
        outputs: ["main"],
        telegramNotify: true,
        properties: [
            {
                ...
                description: "URL of the x402-protected endpoint you want to access (user can customize)",
            },
            {
                displayName: "Network",
                name: "network",
            ...
            },
            {
                ...
                type: "string",
                default: "1.0",
                description: "Maximum USDC amount you're willing to pay (safety limit)",
            },
            {
                ...
                type: "string",
                default: "",
                description: "Token mint address (leave empty to use default USDC mint for the network)",
            },
            {
                ...
                type: "string",
                default: "",
                description: "Custom RPC endpoint (leave empty to use default public endpoints)",
            },
            {
    ...
    async execute(context) {
        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
            try {
                // Get parameters
                const targetUrl = context.getNodeParameter("targetUrl", itemIndex);
                const network = context.getNodeParameter("network", itemIndex);
                const keypairPath = context.getNodeParameter("keypairPath", itemIndex);
                const maxPaymentAmountStr = context.getNodeParameter("maxPaymentAmount", itemIndex);
                const maxPaymentAmount = parseFloat(maxPaymentAmountStr);
                const customTokenMint = context.getNodeParameter("tokenMint", itemIndex);
                const customRpcEndpoint = context.getNodeParameter("rpcEndpoint", itemIndex);
                const method = context.getNodeParameter("method", itemIndex);
                const requestBody = context.getNodeParameter("requestBody", itemIndex);
                ...
                const defaultUSDCMint = network === "devnet"
                    ? "4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU" // Devnet USDC
                    : "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"; // Mainnet USDC
                const tokenMint = customTokenMint || defaultUSDCMint;
                // Default RPC endpoints
                const defaultRpcEndpoint = network === "devnet"
                ...
                const paymentReqs = (await quoteResponse.json());
                const recipientTokenAccount = new PublicKey(paymentReqs.payment.tokenAccount);
                ...
                if (Number(balance.value.amount) < amount) {
                    throw new Error(`Insufficient USDC balance. Have: ${balance.value.uiAmountString}, Need: ${amountUSDC}`);
                }
                ...
                if (!recipientAccountExists) {
                    ...
                    const recipientWallet = paymentReqs.payment.recipientWallet
                        ...
                        : null;
                    if (!recipientWallet) {
                        throw new Error("Recipient token account doesn't exist and recipientWallet not provided in payment requirements");
                    }
                    const createAccountIx = createAssociatedTokenAccountInstruction(payer.publicKey, recipientTokenAccount, recipientWallet, mint);
                    tx.add(createAccountIx);
                    console.log("  + Added create token account instruction");
                }
                // Add transfer instruction
                const transferIx = createTransferInstruction(payerTokenAccount.address, recipientTokenAccount, payer.publicKey, amount);
                tx.add(transferIx);
                // Sign transaction
                tx.sign(payer);
                ...
                const x402Network = network === "devnet" ? "solana-devnet" : "solana-mainnet";
                const paymentProof = {
                    x402Version: 1,
                    scheme: "exact",
                    network: x402Network,
...
//# sourceMappingURL=X402PaymentNode.js.map