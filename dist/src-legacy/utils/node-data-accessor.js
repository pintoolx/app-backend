...
export class NodeDataAccessor {
     ...
     */
    static getAmount(inputData, nodeId) {
        // 檢查輸入資料是否存在
        ...
        // 3. 找不到任何金額欄位
        console.error(`[${nodeId}] Could not find amount field in previous output. Available fields:`, Object.keys(previousOutput));
        return null;
    }
    /**
     * 驗證金額是否有效
     ...
     * @throws Error 如果金額無效
     */
    static validateAmount(value, fieldName) {
        // 檢查類型
        if (typeof value !== 'string' && typeof value !== 'number') {
            throw new Error(`${fieldName} must be string or number, got ${typeof value}: ${JSON.stringify(value)}`);
        }
        // 轉換為 Decimal
        let decimal;
        try {
     ...
     * @returns 是否包含金額
     */
    static hasAmount(inputData) {
        if (!inputData || inputData.length === 0 || !inputData[0]?.json) {
            return false;
        }
        const previousOutput = inputData[0].json;
        return (previousOutput['outputAmount'] !== undefined ||
     ...
     */
    static getValidatedAmount(inputData, nodeId) {
        if (!inputData || inputData.length === 0 || !inputData[0]?.json) {
            return null;
        }
        const previousOutput = inputData[0].json;
     ...
     */
    static parseAmountParameter(paramValue, inputData, currentBalance, nodeId) {
        ...
        if (paramLower === 'auto' || paramValue === '0') {
            ...
            if (amount === null) {
                throw new Error(`Cannot use "auto" or "0": no valid amount from previous node`);
            }
        ...
        if (paramLower === 'all') {
            ...
            const amount = this.getAmount(inputData, nodeId);
            if (amount === null) {
                throw new Error(`Cannot use "all": no current balance and no input from previous node`);
            }
        ...
        if (paramLower === 'half') {
            ...
            const amount = this.getAmount(inputData, nodeId);
            if (amount === null) {
                throw new Error(`Cannot use "half": no current balance and no input from previous node`);
            }
        ...
        try {
            ...
            throw new Error(`Invalid amount parameter "${paramValue}": ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
//# sourceMappingURL=node-data-accessor.js.map