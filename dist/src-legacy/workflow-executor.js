...
export class WorkflowExecutor {
    ...
    telegramNotifier;
    workflowName;
    constructor(telegramNotifier, workflowName) {
     ...
     * @param nodeClass 节点类
     */
    registerNodeType(nodeType, nodeClass) {
     ...
     * 执行单个节点
     */
    async executeNode(workflow, nodeId) {
        // 如果已经执行过，跳过
        ...
        try {
            ...
            if (shouldNotify && this.telegramNotifier?.isEnabled()) {
                await this.telegramNotifier.sendNodeExecutionResult(workflowNode.name, workflowNode.type, result[0], // 发送第一个输出数组的数据
                true);
            }
            // 执行后续节点
