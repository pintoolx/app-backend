-- Strategy Platform Foundation Migration
-- Creates all core strategy tables required by the backend.
-- Must run BEFORE follower-vault and PER migrations.
...
-- 2. strategy_versions: append-only published versions ----------------------
...
-- 3. strategy_deployments: concrete deploy instance -------------------------
CREATE TABLE IF NOT EXISTS public.strategy_deployments (
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  -- Week 4 privacy adapter columns
  er_delegate_signature text,
  er_undelegate_signature text,
  er_router_url text,
  er_committed_at timestamp with time zone,
  umbra_x25519_pubkey text,
  umbra_signer_pubkey text,
  umbra_registration_status text
    CHECK (umbra_registration_status IS NULL OR umbra_registration_status IN ('pending', 'confirmed', 'failed')),
  umbra_register_queue_signature text,
  umbra_register_callback_signature text,
  umbra_master_seed_ref text,
  -- Week 5 PER + Private Payments
  per_endpoint_url text,
  pp_session_id text,
  pp_endpoint_url text,
...
-- 4. strategy_permissions: role grants for non-creator viewers/operators ----
...
-- 5. strategy_public_snapshots: monotonic public-facing summary -------------
...
CREATE INDEX IF NOT EXISTS strategy_public_snapshots_deployment_idx
  ON public.strategy_public_snapshots(deployment_id);
...
-- 7. strategy_treasury_grants: selective disclosure / viewer grants ---------
