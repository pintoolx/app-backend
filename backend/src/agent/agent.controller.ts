...
export class AgentController {
  ...
  @ApiOperation({
    ...
    description:
      'Agent authenticates with wallet signature (same challenge flow as humans) and receives an API key for subsequent requests.',
  })
  ...
  async initWallet(@AgentWallet() walletAddress: string, @Body() dto: AgentInitWalletDto) {
    const account = await this.crossmintService.createAccountWithWallet(
      walletAddress,
      dto.accountName,
      dto.workflowId,
    );
