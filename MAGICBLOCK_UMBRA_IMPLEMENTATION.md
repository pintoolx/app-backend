...
> 本文檔說明 PinTool Backend 中實作的 **MagicBlock（ER / PER / PP）** 與 **Umbra（Encrypted User Account）** 兩大隱私基礎設施模組，以及 Admin Dashboard 如何讓營運團隊即時監控與管理這些元件。
...
1. [架構設計原則](#一架構設計原則)
2. [MagicBlock 三大模組](#二magicblock-三大模組)
   - [2.1 ER — Ephemeral Rollups](#21-er--ephemeral-rollups)
   - [2.2 PER — Private Ephemeral Rollups](#22-per--private-ephemeral-rollups)
   - [2.3 PP — Private Payments](#23-pp--private-payments)
3. [Umbra — Encrypted User Account](#三umbra--encrypted-user-account)
4. [API 端點總覽](#四api-端點總覽)
5. [Admin Dashboard 對應關係](#五admin-dashboard-對應關係)
6. [測試與規格](#六測試與規格)
7. [當前生產狀態](#七當前生產狀態)
8. [總結](#八總結)
...
Admin Dashboard 的 `Overview`、`Privacy`、`System` 三個頁面都會讀取 **Adapter Matrix**，即時顯示每個 adapter 是 `real`（綠色）還是 `noop`（灰色）。營運團隊可一眼確認哪些隱私功能已上線。
...
**相關檔案**
- `backend/src/magicblock/magicblock-er-real.adapter.ts`
- `backend/src/magicblock/magicblock-er-noop.adapter.ts`
- `backend/src/magicblock/magicblock-er.port.ts`
...
- `Privacy` → 點進 Deployment Privacy View 可看到：
  - `er_session_id`
  - `er_router_url`
  - `er_committed_at`
...
**相關檔案**
- `backend/src/magicblock/magicblock-per-real.adapter.ts`
- `backend/src/magicblock/magicblock-per-noop.adapter.ts`
- `backend/src/magicblock/magicblock-per.port.ts`
...
```
1. Client GET  /per/auth/challenge
   → Backend 代理到 TEE /auth/challenge，返回 TEE challenge

2. Client 用 wallet 私鑰簽署 challenge

3. Client POST /per/auth/verify
   → Backend 代理 {challenge, signature, pubkey} 到 TEE /auth/login
...
| `per_auth_tokens` | challenge/token 狀態（Admin Dashboard 直接讀取） |
...
**相關檔案**
- `backend/src/magicblock/magicblock-private-payments-real.adapter.ts`
- `backend/src/magicblock/magicblock-private-payments-noop.adapter.ts`
- `backend/src/magicblock/magicblock-private-payments.port.ts`
...
**相關檔案**
- `backend/src/umbra/umbra-real.adapter.ts`
- `backend/src/umbra/umbra-noop.adapter.ts`
- `backend/src/umbra/umbra-client.service.ts`
- `backend/src/umbra/umbra.port.ts`

Umbra 是基於 Arcium MPC 的加密用戶帳戶協議，提供完整的註冊、金庫、餘額查詢與 viewer 授權能力。
...
所有功能透過 `StrategyDeploymentsController` 暴露為 REST API，受 `JwtAuthGuard` 保護。
...
| 測試檔案 | 對應 Adapter |
|---------|-------------|
| `magicblock-er-real.adapter.spec.ts` | MagicBlock ER |
| `magicblock-per-real.adapter.spec.ts` | MagicBlock PER |
| `magicblock-private-payments-real.adapter.spec.ts` | MagicBlock PP |
| `umbra-real.adapter.spec.ts` | Umbra |
| `umbra-deployment-signer.service.spec.ts` | Umbra Deployment Signer |
...
所有隱私模組處於待命狀態。設定對應環境變數後，MagicBlockModule 會在下次啟動時自動切換為 real adapter，Admin Dashboard 的 Adapter Matrix 也會即時更新為 `real`（綠色）。
