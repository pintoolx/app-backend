...
export async function getMarket({ rpc, marketPubkey }: MarketArgs) {
  ...
  const market = await KaminoMarket.load(rpc as any, marketPubkey, slotDuration);
  if (!market) {
...
export async function getReserveRewardsApy(args: ReserveArgs) {
  ...
  const scope = new Scope('mainnet-beta', args.rpc as any);
  ...
  const farmStates = await FarmState.fetchMultiple(args.rpc as any, [reserve.state.farmDebt, reserve.state.farmCollateral]);
